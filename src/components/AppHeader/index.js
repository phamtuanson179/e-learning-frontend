...
function AppHeader({ brand, transparent, light, action, sticky, relative, center }) {
  ...
  const renderNavbarItems = menuNavbar.map(({ name, icon, href, route, collapse }) => (
  ...
  const renderRoutes = menuNavbar.map(({ name, collapse, columns, rowsPerColumn }) => {
    let template;
    ...
    <Container className="header__wrapper" >
      ...
      >
        ...
        <MKBox
          ...
          px={transparent ? 2 : 0}
        >
          {mobileView && <DefaultNavbarMobile menuNavbar={menuNavbar} open={mobileNavbar} />}
        </MKBox>
      </MKBox>
      {dropdownMenu}
      {nestedDropdownMenu}
