...
const LastestResult = ({ lastestResultExam }) => {
    ...
    return (
        <>
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 4, marginRight: 1 }} className='lastest__result'>
                <Typography variant='h5' component={'div'} sx={{ marginBottom: 4, textAlign: 'center' }}>Lần thi gần nhất</Typography>
                <Box sx={{ display: 'flex', justifyContent: 'space-evenly', alignItems: "center" }}>
                    <Progress
                        type="circle"
                        percent={caculatePercentResult(lastestResultExam?.point, lastestResultExam?.max_point)}
